...
| `evChargerNearby` | boolean? | `true` only when an EV charger (from OSM, and Open Charge Map if configured) sits within 500 m (walking distance). Optional: absent entirely if `cache/ev-chargers.json` was never fetched (`npm run fetch:ev`). See [BUILDING.md](BUILDING.md#stage-4d--ev-charger-proximity-srcfetch-ev-chargersjs). |
